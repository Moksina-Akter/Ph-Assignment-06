    ...
    plants.forEach( plant => {
        plantCardContainer.innerHTML += `
           <div id="${plant.id}" class="p-4 space-y-2 bg-white rounded-md shadow-lg">
                 ...
                 <h4 onclick="details(${plant.id})" class="text-lg font-semibold">${plant.name}</h4>
...
const showAllTrees = (allTrees)=>{
      plantCardContainer.innerHTML ='';
       ...
       plantCardContainer.innerHTML += `
           <div id="${tree.id}"  class="p-4 space-y-2 bg-white rounded-md shadow-lg">
                 <div class="h-[220px]">
                    <img class="w-full h-full rounded " src="${tree.image}" alt="">
                 </div>
                 <h1 onclick="details(${tree.id})"  class="text-lg font-semibold">${tree.name}</h1>
...
const handleDelete = (idDelete) => {
   const remove = allCart.find(cart => cart.id === idDelete)
   if (remove) {
   ...
   if (remove.item === 0) {
      allCart = allCart.filter(carts => carts.id !== idDelete);
   }
